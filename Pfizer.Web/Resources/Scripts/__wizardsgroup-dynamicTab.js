...
function tabStripOnActivate(e) {
    var tabText = $(e.item).find("> .k-link").text();
    debugger;
    if (tabText == undefined) tabText = '';
    var currentModule = $.ajax({
        type: "POST",
        url: _director_fixUrl(window._g_common_Virtualpath + '/Common/AbstractCommon/SetViewBagActiveModule'),
        traditional: true,
        async: false,
        data: {
            tabText: tabText
    ...
    currentModule.done(function (result) {
        return;
        //_director_showWarning("Result: " + result.toString());
    });
};
...
function initTabStrip() {
    ...
    var tab = $(gTabStripNameSelector);
    gTabStrip = tab.data('kendoTabStrip');
    gTabStrip.bind("activate", tabStripOnActivate);
}
...
function _tab_loadModule(myurl, menuText, moduleId) {    
    ...
    var iframeElement = '<iframe id="' + frameId + '" frameborder="0" scrolling="auto" width="100%" height="100%" src="' + url + '" onload="resizeiFrame(\'' + frameId + '\')"></iframe>';
