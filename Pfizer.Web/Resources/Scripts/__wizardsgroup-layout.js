...
/// <reference path="~/Resources/Scripts/__wizardsgroup-customComboBox.js" />
/// <reference path="~/Resources/Scripts/__wizardsgroup-linkedFor.js" />
/// <reference path="~/Resources/Scripts/__wizardsgroup-SingleFieldToMultiselect.js" />
...
function _initKendoControls(dialog) {
    ...
    dialog.find('input[controltype=kendoMaskedTextBox]').each(function () {
        var mask = $(this).attr('mask');
        $(this).kendoMaskedTextBox({ "mask": mask, "rules": {} });
    });
    ...
    dialog.find('input[controltype=kendoAutoCompleteReadOnly]').each(function () {
    ...
    dialog.find('input[controltype=kendoDatePicker]').each(function () {
        ...
        var hasNotInitialize = $(this).parent().find(".k-select").length == 0;
        if (hasNotInitialize) {
    ...
    _configureSingleFieldToMultiselectFor(dialog);
    adjustPopupTabstripSize(dialog);
}